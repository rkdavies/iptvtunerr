...
| **Catch-up recording** | `catchup-record` records current in-progress capsules to local TS files plus `record-manifest.json`, giving non-replay sources a real recorder-backed catch-up path instead of only launcher-mode publishing. |
