...
| **Catch-up capsule preview foundation** | `/guide/capsules.json` and `catchup-capsules` turn real guide rows into previewable near-live capsule candidates with lane, publish, and expiry metadata; `-layout-dir` writes deterministic lane-split files plus a manifest for later publishers. |
