...
Plex Tuner category instances (bcastus, newsus, generalent, etc.) use per-category M3U files served by **iptv-m3u-server**, e.g. `http://iptv-m3u-server.plex.svc/dvr-bcastus.m3u`. For those lineups and guides to be non-empty and for streams to work, the **split** step that generates `dvr-*.m3u` must satisfy the following.
...
- Plex Tuner’s M3U parser already supports multiple URL lines per EXTINF: it collects every `http(s)://` line after an EXTINF into that channel’s `StreamURLs` list. Dedupe-by-tvg-id then merges duplicates; `PLEX_TUNER_STRIP_STREAM_HOSTS` then strips CF hosts. Channels that have at least one non-CF URL remain; channels whose only URLs are on blocked hosts are dropped.
...
2. Write one EXTINF block per channel with **all** of those URL lines following it (same M3U format as Plex Tuner’s parser expects: EXTINF, then one or more `http...` lines before the next EXTINF or end of file).
...
```bash
kubectl exec deploy/plextuner-supervisor -n plex -- sh -c 'curl -s http://iptv-m3u-server.plex.svc/dvr-bcastus.m3u | grep -E "^http" | sed "s|.*://||;s|/.*||" | sort -u'
```
...
- [plextuner-troubleshooting.md](../runbooks/plextuner-troubleshooting.md) — §10 Category DVRs empty.
