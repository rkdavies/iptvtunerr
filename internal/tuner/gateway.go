...
func (g *Gateway) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for i, streamURL := range urls {
		...
		if isHLSResponse(resp, streamURL) {
			...
			skipFFmpeg := strings.EqualFold(os.Getenv("IPTV_TUNERR_FFMPEG_DISABLED"), "1") ||
				strings.EqualFold(os.Getenv("IPTV_TUNERR_FFMPEG_DISABLED"), "true") ||
				strings.EqualFold(os.Getenv("IPTV_TUNERR_FFMPEG_DISABLED"), "yes")
			if !skipFFmpeg {
